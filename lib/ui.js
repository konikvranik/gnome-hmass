 ...
 * = výška textu v panelu. Ikona se svisle centruje v řádku (stejně jako
 * label - viz createPanelEntity), aby střely na textovou lajnu.
...
function createPanelEntity(entityId, stateObj, ha, cfg) {
    ...
    // Ikona pro panel: 18 px, svisle centrovANá (makeEntityIcon); bez
    // posunů - na střed řádku se centruje i text (panelLabel níže)
    const mkPanelIcon = () => makeEntityIcon(domain, stateObj, cfg.icon, 18);

    // Label pro panel: musí se centrovat explicitně - jinak vyplní celou
    // výšku panelu a text se kreslí od horního okraje (text "vyjede" nahoru)
    const panelLabel = (text) => {
        const l = new St.Label({text: text || '', style_class: 'hmass-panel-value'});
