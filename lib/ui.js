...
function _displayRow(entityId, stateObj, decimals) {
    const domain = entityId.split('.')[0];
    const row = _nonReactiveRow();
    const icon = new St.Icon({
        icon_name: DOMAIN_ICONS[domain] || 'text-x-generic-symbolic',
        icon_size: 16,
        y_align: Clutter.ActorAlign.CENTER,
    });
    ...
    const update = st => {
        ...
        name.text = (st.attributes && st.attributes.friendly_name) || entityId;
        const unit = st.attributes && st.attributes.unit_of_measurement;
        const num = parseFloat(st.state);
        if (!isNaN(num) && isFinite(num)) {
            const txt = formatNumericState(st, decimals);
 ...
 */
function createHaRows(entityId, stateObj, ha, decimals) {
    const domain = entityId.split('.')[0];
    const rows = [];
    const updaters = [];
    const name = st => (st && st.attributes && st.attributes.friendly_name) || entityId;
    ...
    if (TOGGLE_DOMAINS.has(domain)) {
        ...
        const d = _displayRow(entityId, stateObj, decimals);
        rows.push(d.row);
...
function _showPanelTooltip(actor, text) {
    ...
    // panel je na horní hraně obrazovky - tooltip patří pod widget
    ...
    _tipLabel.set_position(tx, Math.round(ay + (actor.height || 0) + 8));
}
 ...
 */
function createPanelEntity(entityId, stateObj, ha, decimals) {
    const domain = entityId.split('.')[0];
    const name = st => (st && st.attributes && st.attributes.friendly_name) || entityId;
    ...
    if (TOGGLE_DOMAINS.has(domain)) {
        ...
        update = st => {
            ...
            icon.opacity = dead ? 25 : (st.state === 'on' ? 255 : 110);
        };
        ...
        const btn = new St.Button({can_focus: false, style_class: 'hmass-panel-btn'});
        const icon = new St.Icon({
            icon_name: DOMAIN_ICONS[domain] || 'media-playback-start-symbolic',
            icon_size: 16,
        });
        btn.set_child(icon);
        btn.connect('clicked', () =>
        ...
        const icon = new St.Icon({
            icon_name: DOMAIN_ICONS[domain] || 'preferences-system-symbolic',
            icon_size: 14,
        });
        ...
        update = st => {
            ...
            if (!isNaN(v))
                current = v;
            label.text = panelValueText(st, decimals);
        };
        attachPanelTooltip(box, () =>
            `${name(last)} — ${panelValueText(last, decimals)} (kolečko mění hodnotu)`);
    } else if (SELECT_DOMAINS.has(domain)) {
        const btn = new St.Button({can_focus: false, style_class: 'hmass-panel-btn'});
        const box = new St.BoxLayout({style_class: 'hmass-panel-select'});
        const icon = new St.Icon({
            icon_name: DOMAIN_ICONS[domain] || 'open-menu-symbolic',
            icon_size: 14,
        });
        const label = new St.Label({style_class: 'hmass-panel-value'});
        box.add_child(icon);
