 ...
 * gicon/icon_name se předává hned v konstruktoru - na GNOME 42 St.Icon
 * s giconem nastaveným dodatečně (po konstrukci) texturu nenačte.
 */
function makeEntityIcon(domain, stateObj, iconMode, size) {
    ...
    let lastDef = first;
    // DIAG: ladění ikon v liště
    log(`hmass DIAG icon ${domain}/${iconMode}: ` +
        `${first === null ? 'text' : (first.gicon
            ? 'gicon ' + first.gicon.to_string() : 'iconName ' + first.iconName)}`);
    const sync = st => {
...
function createPanelEntity(entityId, stateObj, ha, cfg) {
    ...
    if (TOGGLE_DOMAINS.has(domain)) {
        ...
        const ic = makeEntityIcon(domain, stateObj, cfg.icon, 14);
